...
enum class LevelEvent(val id: Int) {
    ...
    companion object {
        fun fromId(id: Int): LevelEvent {
            return entries.first { it.id == id }
        }

        val STREAM_CODEC = object : StreamCodec<LevelEvent> {
            override fun write(buffer: ByteBuf, value: LevelEvent) {
                buffer.writeInt(value.id)
            }

            override fun read(buffer: ByteBuf): LevelEvent {
                return fromId(buffer.readInt())
            }
        }
    }
}