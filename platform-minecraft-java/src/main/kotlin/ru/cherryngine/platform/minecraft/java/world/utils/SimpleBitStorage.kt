package ru.cherryngine.platform.minecraft.java.world.utils
...
class SimpleBitStorage(
    ...
    init {
        ...
        if (data != null) {
            if (data.size != dataSize) throw IllegalArgumentException("Invalid storage length: $dataSize (expected ${data.size})")
            this.data = data
        } else {
    ...
    override fun equals(other: Any?): Boolean =
        this === other || other is SimpleBitStorage && bits == other.bits && size == other.size && data.contentEquals(
            other.data
        )
