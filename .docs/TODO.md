...
- [ ] **`PlayerManager` per-instance** — после реализации `ConnectionRegistry` перевести `PlayerManager` в per-instance (через `InstanceScope`). `onlinePlayers()` должен возвращать только игроков текущего инстанса.

- [ ] **`PlayerService` и `WorldService`** — переосмыслить. Сейчас написаны наспех. `LoginPacket` содержит данные от игровой логики — нужен чистый механизм для этого.

- [ ] **`@InstanceSingleton`** — заменить ручной `InstanceScope` на Micronaut custom scope. Бины помеченные `@InstanceSingleton` создаются lazy per-instance и уничтожаются вместе с инстансом.
...
- [ ] **Bedrock чанки** — `BedrockDemoInstanceSetup.createTickables()` возвращает `emptyList()`. Реализовать `BedrockViewTickable` по аналогии с `MinecraftViewTickable`.

- [ ] **Bedrock entity** — реализовать `BedrockAxolotlRenderer` и `BedrockCubeRenderer`.
...
- [ ] **Сложные формы блоков в физике** — ступеньки, заборы, полублоки.
