...
class RegistryException(val identifier: String, val mapSize: Int): Exception() {
    ...
    override val message: String
        get() {
            return if(mapSize == 0) {
                "Registry belonging to identifier/protocol id $identifier is not initialized yet! (Make sure to create the MinecraftServer object before using registries, listening on events or registering commands)"
            } else {
