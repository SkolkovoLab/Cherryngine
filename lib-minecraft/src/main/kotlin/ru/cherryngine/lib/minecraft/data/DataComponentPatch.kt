...
class DataComponentPatch(
    internal val components: Int2ObjectMap<DataComponent?>,
    val isPatch: Boolean,
        ...
        fun read(buffer: ByteBuf, isPatch: Boolean, isTrusted: Boolean): DataComponentPatch {
            ...
            for (i in 0 until added) {
                ...
                if (isTrusted) {
                    ...
                    val array = StreamCodec.BYTE_ARRAY.read(buffer)
                    val tempBuffer = Unpooled.copiedBuffer(array)
                    patch.put(id, streamCodec.read(tempBuffer))
                }
            }
    ...
    fun write(buffer: ByteBuf) {
        ...
        StreamCodec.VAR_INT.write(buffer, added)

        // amount of new components added
        if (isPatch) {
            StreamCodec.VAR_INT.write(buffer, components.size - added)
        }
        ...
        components.int2ObjectEntrySet().forEach { entry ->
            ...
            if (isTrusted) {
                ...
                val componentData = byteBufBytes { b ->
                    streamCodec.write(b, value)
                }
                StreamCodec.BYTE_ARRAY.write(buffer, componentData)
            }
        }
