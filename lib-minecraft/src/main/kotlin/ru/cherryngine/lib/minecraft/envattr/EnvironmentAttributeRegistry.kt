...
object EnvironmentAttributeRegistry {
    private val map = hashMapOf<Key, EnvironmentAttribute<*>>()

    operator fun get(key: Key): EnvironmentAttribute<*> {
        return map[key]!!
    }
    ...
    fun <T> register(identifier: String, type: EnvironmentAttributeType<T>, default: T) {
        map[identifier.toKey()] = EnvironmentAttribute("minecraft:$identifier", type, default)
    }
}
